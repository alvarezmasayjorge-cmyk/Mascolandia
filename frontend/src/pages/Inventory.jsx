...
import { Search, Plus, X, Package, ArrowUpCircle, ArrowDownCircle } from 'lucide-react';
...
export default function Inventory() {
  ...
  const filteredItems = items.filter(i => {
    ...
    if (filterStatus === 'alert') return matchSearch && (status !== 'OK');
    return matchSearch;
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
        ...
        <button onClick={() => { setFormData({ name: '', category: 'Medicamento', stock: '', unit: 'Unidad', expiryDate: '', costPrice: '', sellingPrice: '', supplier: '', notes: '' }); setIsModalOpen(true); }}
      ...
      <div className="flex gap-3">
        ...
        <select className="bg-white border border-gray-100 rounded-xl px-4 text-sm text-ink-700 outline-none" value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
            ...
            <tbody>
              {filteredItems.map(item => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-surface-sunken transition-colors">
                  ...
                  <td className="p-4 text-ink-900">{item.sellingPrice ? `$${item.sellingPrice}` : '-'}</td>
                  <td className="p-4">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-xl font-display font-bold text-ink-900">Nuevo producto</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-ink-700 mb-1 font-medium">Stock inicial</label>
                ...
                <div>
                  <label className="block text-sm text-ink-700 mb-1 font-medium">Costo ($)</label>
                ...
                <div>
                  <label className="block text-sm text-ink-700 mb-1 font-medium">Precio venta ($)</label>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleMovement} className="p-6 space-y-4">
              <div>
                ...
                <p className="text-xs text-ink-300 mt-1">Stock actual: {movementModal.stock} {movementModal.unit}</p>
              </div>
              {movementData.type === 'ENTRADA' && (
                <>
                  ...
                  {movementData.registerExpense && (
                    <div>
                      <label className="block text-sm text-ink-700 mb-1 font-medium">Costo total ($)</label>
